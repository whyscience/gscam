...
class IPCamNode:
    def __init__(self):
        ...
        # Parameters
        self.video_file = rospy.get_param('~video_file', '/home/eric/Videos/wall.mp4')
        ...
        self.calibration_file = rospy.get_param('~calibration_file', os.path.expanduser('~/.ros/camera_info/camera.yaml'))
        ...
        if not os.path.exists(self.video_file):
            rospy.logerr("Video file does not exist: %s", self.video_file)
            return
        self.cap = cv2.VideoCapture(self.video_file)
        if not self.cap.isOpened():
            rospy.logerr("Failed to open video file: %s", self.video_file)
            return
    ...
    def run(self):
        while not rospy.is_shutdown():
            try:
                ...
                if not ret:
                    rospy.loginfo("Reached end of video file, restarting from beginning")
